...
- [ ] Commit explicit paths with hooks enabled. Require empty `git status --porcelain` and passing `node scripts/guard-ota-clean-tree.js`.
- [ ] Push the clean repair branch to the maintained GitHub repository and run CI on that exact SHA. Do not conflate a GitHub push with production publication.
...
- Security lockfile updates are limited to compression 1.8.2, multer 2.4.0, proxy-addr 2.0.8, moment 2.31.0 and undici 6.28.1 where present. `node-forge` 1.4.0 has no published fixed successor as of this check; exposure/remediation review is open. No security exception or ignore was added.
